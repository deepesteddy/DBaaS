...
sequence_count = int(os.environ['SEQ_COUNT'])    #sequence count
sequenceID = 0

if sequence_count > 1:
	indexOfDash = node_name.index("-")  #locate index of '-', node ID follows
	sequenceID = int(node_name[(indexOfDash + 1):])
...
def openChannels(files, handles):
	...
	for inp_file in os.listdir(inp_dir):
		...
		sortID = int(inp_file[(index + 1):])
    
		#debug
		#sys.stderr.write(node_name + ": , sortID: " + str(sortID) + '\n')
    
	          		
        # check if sorted is connected to this sequence, if so open file handle
		if sequence_count == 1:
			...
			remainder = sortID % sequence_count
			if ( (remainder == sequenceID) or ((remainder == 0) and (sequenceID == sequence_count))  ) :
				fp = open(os.path.join(inp_dir, inp_file), 'rb', buffering=(2<<18) + 8)
				handles.append(fp)
				files.append(inp_file)
...
#DEBUG
#list inp_files for this sequence/name node      
...
def getMinValue(handles, tupples, keys):
...
#open channel to join node
joinChannel = open('/dev/out/join', 'ab', buffering=(2<<18) + 8)
...
while flag:
	...
	try:
      	#get tupple with min key value
		tupple = getMinValue(fileHandleList, listOfChannelTupples, listOfKeys)
        
        #write tuple to join node
		joinChannel.write(str(tupple))
